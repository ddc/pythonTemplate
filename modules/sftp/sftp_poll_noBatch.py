...
class SFTP:
    ...
    def _get_paramiko_connection(self, file_src):
        ...
        try:
            _window_size = pow(2, 24)
            _max_packet_size = pow(2, 24)
            transport = paramiko.Transport(self.host, self.port)

            if self.pkey_file is not None and os.path.isfile(self.pkey_file):
                paramiko_key = paramiko.RSAKey.from_private_key_file(self.pkey_file)
                transport.connect(username=self.user, pkey=paramiko_key)
            else:
                passw = self.password
                if passw is None or len(passw) == 0:
                    error_msg = f"{messages.SFTP_CONN_ERROR}: {messages.INVALID_PASSWORD}: {file_src}"
            ...
            try:
                ...
                error_msg = f"[{messages.SFTP_REMOTE_DIR_NOT_FOUND}]:{utils.get_exception(e)}: {file_src} -> {remote_dir}"
                return {"sftp": None, "transport": None, "error_msg": error_msg}
        except Exception as e:
            error_msg = f"[{messages.SFTP_CONN_ERROR}]:{utils.get_exception(e)}"
            return {"sftp": None, "transport": None, "error_msg": error_msg}
    ...
    def start_send_sftp_pool(self, file_src, file_dst):
        """
            /usr/bin/sftp -qpP 22 -i /home/vagrant/.ssh/id_rsa vagrant@127.0.0.1:/tmp <<< $'put /home/vagrant/test.csv
                /tmp/test.csv_part\n rename /tmp/test.csv_part /tmp/test.csv'
            -q quiet
        ...
        if not os.path.exists(self.linux_sftp_bin):
            msg = f"{log_header_msg}:{messages.SFTP_BIN_NOT_FOUND}: {self.linux_sftp_bin}"
            return {"result": False, "level": "ERROR", "msg": msg, "filename": None}

        if not os.path.exists(self.pkey_file):
            msg = f"{log_header_msg}:{messages.SFTP_PKEY_NOT_FOUND}: {self.pkey_file}"
            return {"result": False, "level": "ERROR", "msg": msg, "filename": None}

        if os.name != "nt":
            pkey_permissions = oct(os.stat(self.pkey_file).st_mode)[-4:]
            if pkey_permissions != "0600":
                error_msg += f"[{messages.SFTP_PKEY_WRONG_PERMS}]: " \
                             f"[{messages.PERMISSION_CURRENT}: {pkey_permissions}]:" \
                             f"[{self.pkey_file}]: {self.pkey_file}"
                try:
                    os.chmod(self.pkey_file, 0o600)
                    error_msg += f"[{messages.SFTP_PKEY_CHANGED_PERMS}]: " \
        ...
        try:
            buffer_size = pow(2, 16)
            cmd = f"{self.linux_sftp_bin} -B {buffer_size} -vpP {self.port} -i {self.pkey_file} {user_host}:" \
                  f"{self.remote_dir} <<< $'put {file_src} {temp_file_dst}\n rename {temp_file_dst} {file_dst}'"

            process = subprocess.Popen(cmd, shell=True, executable="bash", universal_newlines=True,
            ...
            try:
                ...
                error_msg += f"{log_header_msg}:{messages.FILE_REMOVE_ERROR}:{utils.get_exception(e)}: {file_src}"

            msg = f"[SFTP]:[{messages.SFTP_TRANSFERED}]:" \
                  f"[{self.full_host}]:" \
                  f"[{size_bytes}]: " \
                  f"{file_src} -> {file_dst}"
