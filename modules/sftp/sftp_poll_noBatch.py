...
class SFTP:
    ...
    def start_send_sftp_pool(self, file_src, file_dst):
        """
            /usr/bin/sftp -qpP 22 -i /home/vagrant/.ssh/id_rsa
                vagrant@127.0.0.1:/tmp <<< $'put /home/vagrant/test.csv
        ...
        try:
            ...
            cmd = f"{self.linux_sftp_bin} -B {buffer_size} -vpP {self.port} " \
                  f"-i {self.pkey_file} {user_host}:{self.remote_dir} " \
                  f"<<< $'put {file_src} {temp_file_dst}\n " \
                  f"rename {temp_file_dst} {file_dst}'"
            ...
            try:
                ...
                error_msg += f"{log_header_msg}:" \
                             f"{messages.FILE_REMOVE_ERROR}:" \
                             f"{utils.get_exception(e)}: {file_src}"
