...
class Log:
    ...
    def setup_logging(self):
        ...
        try:
            ...
            sys.stderr.write("{}:{}: {}\n".format(
                messages.LOG_FILE_NOT_WRITABLE,
                utils.get_exception(e),
        ...
        if self.level == logging.DEBUG:
            ...
            formatt = "[%(asctime)s.%(msecs)03d]:[%(levelname)s]:" \
                      "[PID:{}]:%(message)s".format(os.getpid())
...
class GZipRotator:
    def __init__(self, dir_logs, log_file_path, days_to_keep, rotation_days):
        ...
        if os.path.isfile(log_file_path) \
                ...
                and utils.is_file_older_than_x_days(log_file_path,
                                                    rotation_days):
            file_creation_time = (
                datetime.utcfromtimestamp(os.path.getctime(log_file_path)))\
                .strftime("%Y%m%d")
            fname_bkp, ext_log = os.path.splitext(
                os.path.basename(log_file_path)
            )
            log_backup_name = "{}_{}{}".format(fname_bkp,
                                               file_creation_time,
                                               ext_log)
            log_backup_path = os.path.join(dir_logs, log_backup_name)

            try:
                with open(log_file_path, "rb") as fin:
                    with gzip.open("{}.gz".format(log_backup_path),
                                   "wb") as fout:
                        fout.writelines(fin)
            except Exception as e:
                sys.stderr.write("{}:{}: {}\n".format(
                    messages.LOG_COMPRESS_ERROR,
                    utils.get_exception(e),
                    os.path.normpath(log_backup_path)
                ))
                try:
                    os.rename(log_file_path, log_backup_path) \
                        if os.path.isfile(log_file_path) else None
                except IOError as e:
                    sys.stderr.write("{}: {}\n".format(
                        messages.LOG_RENAME_ERROR,
                        utils.get_exception(e)
                    ))
                return

            try:
                os.remove(log_file_path) if os.path.isfile(log_file_path) \
                    else None
            except Exception as e:
                sys.stderr.write("{}:{}: {}\n".format(
                    messages.LOG_REMOVE_ERROR,
                    utils.get_exception(e),
                    os.path.normpath(log_file_path)
                ))
...
class RemoveOldLogs:
    def __init__(self, dir_logs, days_to_keep):
        if os.path.isdir(dir_logs):
            files_list = [f for f in os.listdir(dir_logs) if
                          os.path.isfile(os.path.join(dir_logs, f))
                          and f.lower().endswith(".gz")]
            for f in files_list:
                full_path = os.path.join(dir_logs, f)
                if utils.is_file_older_than_x_days(full_path, days_to_keep):
                    try:
                        os.remove(full_path) if os.path.isfile(full_path) \
                            else None
                    except OSError as e:
                        sys.stderr.write("{}:{}: {}\n".format(
                            messages.LOG_REMOVE_ERROR,
